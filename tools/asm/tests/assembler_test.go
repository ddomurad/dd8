...
func TestThatCanBuildBinaryCode(t *testing.T) {
	...
	t.Run("rmb", func(t *testing.T) {
		assertAssembler(t, "rmb 0x19,0", []byte{0x07, 0x19})
		assertAssembler(t, "rmb 0x19,1", []byte{0x17, 0x19})
		assertAssembler(t, "rmb 0x19,2", []byte{0x27, 0x19})
		assertAssembler(t, "rmb 0x19,3", []byte{0x37, 0x19})
		assertAssembler(t, "rmb 0x19,4", []byte{0x47, 0x19})
		assertAssembler(t, "rmb 0x19,5", []byte{0x57, 0x19})
		assertAssembler(t, "rmb 0x19,6", []byte{0x67, 0x19})
		assertAssembler(t, "rmb 0x19,7", []byte{0x77, 0x19})
	})
	t.Run("php", func(t *testing.T) {
		assertAssembler(t, "php", []byte{0x08})
	})
	t.Run("bbr", func(t *testing.T) {
		assertAssembler(t, "bbr 0x10, 0", []byte{0x0f, 0x0e})
		assertAssembler(t, "bbr 0x10, 1", []byte{0x1f, 0x0e})
		assertAssembler(t, "bbr 0x10, 2", []byte{0x2f, 0x0e})
		assertAssembler(t, "bbr 0x10, 3", []byte{0x3f, 0x0e})
		assertAssembler(t, "bbr 0x10, 4", []byte{0x4f, 0x0e})
		assertAssembler(t, "bbr 0x10, 5", []byte{0x5f, 0x0e})
		assertAssembler(t, "bbr 0x10, 6", []byte{0x6f, 0x0e})
		assertAssembler(t, "bbr 0x10, 7", []byte{0x7f, 0x0e})
	})
	...
	t.Run("smb", func(t *testing.T) {
		assertAssembler(t, "smb 0x19,0", []byte{0x87, 0x19})
		assertAssembler(t, "smb 0x19,1", []byte{0x97, 0x19})
		assertAssembler(t, "smb 0x19,2", []byte{0xa7, 0x19})
		assertAssembler(t, "smb 0x19,3", []byte{0xb7, 0x19})
		assertAssembler(t, "smb 0x19,4", []byte{0xc7, 0x19})
		assertAssembler(t, "smb 0x19,5", []byte{0xd7, 0x19})
		assertAssembler(t, "smb 0x19,6", []byte{0xe7, 0x19})
		assertAssembler(t, "smb 0x19,7", []byte{0xf7, 0x19})
	})
	t.Run("bbs", func(t *testing.T) {
		assertAssembler(t, "bbs 0x10, 0", []byte{0x8f, 0x0e})
		assertAssembler(t, "bbs 0x10, 1", []byte{0x9f, 0x0e})
		assertAssembler(t, "bbs 0x10, 2", []byte{0xaf, 0x0e})
		assertAssembler(t, "bbs 0x10, 3", []byte{0xbf, 0x0e})
		assertAssembler(t, "bbs 0x10, 4", []byte{0xcf, 0x0e})
		assertAssembler(t, "bbs 0x10, 5", []byte{0xdf, 0x0e})
		assertAssembler(t, "bbs 0x10, 6", []byte{0xef, 0x0e})
		assertAssembler(t, "bbs 0x10, 7", []byte{0xff, 0x0e})
	})
